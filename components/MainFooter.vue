...
export default {
  ...
  async fetch() {
    return this.$storyblok.get(`cdn/stories/global/globals`).then(res => {
      // FIXME: Temporary filter to remove EY references from footer links until we can access the CMS
      const globals = {
        ...res.data.story,
        content: {
          ...res.data.story.content,
          footer_links: res.data.story.content.footer_links.filter(
            link => !['Privacy Policy', 'LinkedIn'].includes(link.text)
          )
        }
      }

      this.$set(this, 'globals', globals)
    })
  },
  methods: {
    ToggleModal() {
